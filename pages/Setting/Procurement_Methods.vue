...
import Procurement_Methods from "~/assets/scripts/objects/Procurement_Methods";
...
export default {
    ...
    return {
      ...
      form: new DefaultForm({
        ...
        ShowForm: (title, isAdd, obj) => {
          ...
          this.form.obj = new Procurement_Methods(obj);
  ...
  methods: {
    ...
    Add() {
      this.form.ShowForm("Add Procurement Methods", true);
    },
    Edit(row) {
      this.form.ShowForm("Edit Procurement Methods", false, row);
    },
    Delete(row) {
      ShowConfirm({
        ...
        .then(() => {
          GetDataAPI({
            method: "POST",
            url: API.Delete_Procurement_Methods,
            params: row,
            action: (re) => {
              if (re == "OK") {
                this.LoadData();
      ...
      this.$refs.form.getValidate().then((re) => {
        if (!re) {
          ...
          GetDataAPI({
            method: "post",
            url: this.isAdd
              ? API.Add_Procurement_Methods
              : API.Edit_Procurement_Methods,
