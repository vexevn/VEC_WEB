...
export default {
      ...
      tp: new TablePaging({
        ...
        cols: [
          ...
          new TablePagingCol({
            ...
            data: "OfficeType",
            min_width: 120,
            formatter: (value) => Para.OfficeType.getName(value),
          }),
          new TablePagingCol({
            title: "Sử dụng",
            data: "Use",
  ...
  methods: {
    ...
    Add() {
      this.form.ShowForm("Thêm văn phòng mới", true);
    },
    Edit(row) {
      this.form.ShowForm("Sửa văn phòng", false, row);
    },
    Delete(row) {
      ShowConfirm({
    ...
    Save() {
      ...
      this.$refs.form.getValidate().then((re) => {
        if (!re) {
          ...
          GetDataAPI({
            method: "post",
            url: this.isAdd ? API.Add_Office : API.Edit_Office,
