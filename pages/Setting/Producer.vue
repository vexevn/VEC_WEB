...
import producer from "~/assets/scripts/objects/dm_producer";
...
export default {
  data() {
    return {
      ...
      tp: new TablePaging({
        title: "Title",
        data: APIHelper.producer.GetList,
      ...
      form: new DefaultForm({
        ...
        ShowForm: (title, isAdd, obj) => {
          ...
          this.form.obj = new producer(obj);
  ...
  methods: {
    ...
    Delete(row) {
      ShowConfirm({
        ...
        .then(() => {
          APIHelper.producer.Delete(row).then((re) => {
            this.LoadData();
    ...
    Save() {
      ...
      this.$refs.form.getValidate().then((re) => {
        if (!re) {
          ...
          if (!this.form.obj.Id) {
            APIHelper.producer.Add(this.form.obj.toJSON()).then((re) => {
              this.LoadData();
              this.form.visible = false;
              ShowMessage("Lưu thành công");
            });
          } else {
            APIHelper.producer.Edit(this.form.obj.toJSON()).then((re) => {
              this.LoadData();
              this.form.visible = false;
              ShowMessage("Lưu thành công");
            });
